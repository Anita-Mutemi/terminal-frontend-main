...
const GranularSearch = () => {
  ...
  const handleChange = (selectedOption: any, inputName) => {
    // setSelectedValues({
    //   ...selectedValues,
    //   [inputName]: selectedOption.map((item: any) => item.value),
    // });
    setSelectedValues({
      ...selectedValues,
  ...
  //   },
  //   {
  //     company_name: 'Chargeway',
  //     website: 'https://chargeway.net/',
  //   },
  //   {
  ...
  //   },
  //   {
  //     company_name: 'Electrify America',
  ...
  //   },
  //   {
  //     company_name: 'SemaConnect',
  //     website: 'https://www.semaconnect.com/',
  //   },
  //   {
  //     company_name: 'FLO',
  ...
  //   {
  //     "id": "423c10ab-3271-4163-9721-f39aaf4e3b13",
  //     "value": "423c10ab-3271-4163-9721-f39aaf4e3b13",
  //     "label": "Okibo",
  //     "color": "",
  //     "title": "Okibo",
  //     "uuid": "423c10ab-3271-4163-9721-f39aaf4e3b13",
  //     "description": "Okibo is developing intelligent, mobile, multi-purpose and autonomous robots for use in construction sites. The company’s first product is a robot for wall rendering (e.g., stucco, EIFS, concrete, primer, and adhesives) that will be used to simplify and lower costs of handling wall isolation.`​\nOkibo’s robots sit atop a highly mobile platform and use advanced computer vision, laser detectors, geo-positioning, and inertial motion sensors to operate at, and assimilate into any construction site. The robot’s versatile robotic manipulators enable the programming of a multitude of tasks that are today performed by manual labor or limited machinery.",
  ...
  const debouncedLoadOptions = async (inputValue: string) => {
    const response = await axios.get(
      `/v1/search/suggest_tags?raw_query_str=${inputValue}`,
      {
        headers: {
    ...
    return data.map((keyword: { id: any; name: any }) => ({
      // @ts-ignore
      id: keyword.value,
      // @ts-ignore
      value: keyword,
      // @ts-ignore
      label: `${keyword.value} [${keyword.type}]`,
      // @ts-ignore
      color: keyword.type === 'keyword' ? '#9098b1' : '#db6f56',
    }));
  };

  const debouncedLoadVerticals = async (inputValue: string) => {
    const response = await axios.get(
      `/v1/search/options/verticals?query_str=`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
        },
  ...
  const debouncedLoadSuggestions = async (
    ...
    const response =
      inputName !== 'project'
        ? await axios.get(
            `/v1/search/${
              inputName === 'project' ? 'search' : 'options'
            }/${inputName}?${
  ...
  const getFilters = async (project_uuid: string) => {
    ...
    const response = await axios.post(
      `/v1/search/search/project/${project_uuid}/filters`,
      {},
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
  ...
  const debouncedLoadCompanies = async (
    ...
    const response = await axios.get(
      `/v1/search/options/${inputName}?query_str=${inputValue}`,
      {
        headers: {
          Authorization: `Bearer ${access_token}`,
        },
